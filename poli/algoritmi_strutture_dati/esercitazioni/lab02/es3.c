...
tratte *read_file(int *len, char str[]);
...
int main(void)
{
    ...
    tratte *logs = read_file(&n_logs, FILE_PATH_SOURCE);
...
void selezionatiDati(tratte **logs, int *n, comando_e command)
{
    ...
    case r_lettura:
        ...
        *logs = read_file(n, str);

        sorted_date = (tratte **)malloc((*n) * sizeof(tratte *));
        sorted_codice_tratta = (tratte **)malloc((*n) * sizeof(tratte *));
        sorted_partenza = (tratte **)malloc((*n) * sizeof(tratte *));
        sorted_capolinea = (tratte **)malloc((*n) * sizeof(tratte *));
